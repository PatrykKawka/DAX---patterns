...
2. Filtr ten propaguje się przez relacje z `dim_Kalendarz` i `dim_Klienci` do `fact_Sprzedaz` (kierunek Single).
...
**Użycie miary wewnątrz iteratora i tak automatycznie owija ją w `CALCULATE`, dlatego nie ma znaczenia czy zrobimy to jawnie czy nie. Jeśli wewnątrz iteratora wpiszemy treść miary jest to interpretowane jako wyrażenie a nie miara dlatego trzeba to owinąć w `CALCULATE`**.
